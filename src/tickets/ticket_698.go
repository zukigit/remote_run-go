...
func CheckWhetherTheJobIconRunsNormally(jobnetId string, testcase *dao.TestCase, targetJobnetStatus string, targetJobStatus string, processCheckTimeout int, fileName string, sshClient *ssh.Client) common.Testcase_status {
	...
	if testcase.Get_no() == 4 {
		...
		if err != nil {
			fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s Failed to check file existence and size.", err.Error()))
			return FAILED
		}
...
func AgentConnectionReconnection(jobnetId string, sleepCount int, testcase *dao.TestCase, targetJobnetStatus string, targetJobStatus string, processCheckTimeout int, fileName string, sshClient *ssh.Client) common.Testcase_status {
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to sleep your linux os. Sleep count: %d", err.Error(), sleepCount))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Successfully sleep your linux os in bg. Sleep count: %d", sleepCount))
	...
	if testcase.Get_no() == 8 || testcase.Get_no() == 9 {
		...
		if err != nil {
			fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error ssh connection connect failed."))
			return FAILED
		}
		fmt.Print(lib.Logi(common.LOG_LEVEL_INFO, "Successfully reboot hostname : %s", result2))
	}
...
func AllowRootFileTransfer(jobnetId string, testcase *dao.TestCase, targetJobnetStatus string, targetJobStatus string, processCheckTimeout int, sshClient *ssh.Client) common.Testcase_status {
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s Failed to set jobarg agent config for AllowRoot 0.", err.Error()))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Jobarg agent config change for parameter AllowRoot 0 is success."))

	// Attempt to restart the agent initially
	err = lib.Restart_jaz_agent_linux()
	if err == nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Agent restarted successfully."))
	} else {
		// Log initial restart failure
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to restart the agent. Proceeding with cleanup...", err.Error()))
		...
		for _, cmd := range cleanupCommands {
			...
			if err != nil {
				fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to execute cleanup command: %s", err.Error(), cmd))
				return FAILED
			}
			fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Cleanup successful for command: %s", cmd))
		}

		// Attempt to restart the agent again after cleanup
		err = lib.Restart_jaz_agent_linux()
		if err != nil {
			fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to restart the agent after cleanup.", err.Error()))
			return FAILED
		}
		fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Agent restarted successfully after cleanup."))
	}
	...
	_, err = lib.Ssh_exec_to_str(sedCmd)
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s Failed to set jobarg agent config for AllowRoot 1.", err.Error()))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Jobarg agent config change for parameter AllowRoot 1 is success."))
...
func RebootAfterCompletingJob_30(jobnetId string, testcase *dao.TestCase, normalJobnetStatus string, normalJobStatus string, rebootJobnetStatus string, rebootJobStatus string, processCheckTimeout int, sshClient *ssh.Client) common.Testcase_status {
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, std_out: %s", err.Error(), normalJobnetId))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "%s has been successfully run with registry number: %s", jobnetId, normalJobnetId))

	// Enable the Reboot Jobnet
	if err := lib.Jobarg_enable_jobnet("Icon_1", "reboot_after_job_completing_jobs_timeout_30"); err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Failed to enable reboot jobnet, Error: %s", err))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Enable the reboot jobnet success."))

	// Run reboot jobnet
	rebootJobnetId, err := lib.Jobarg_exec_E(jobnetId, envs)
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, std_out: %s", err.Error(), rebootJobnetId))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "%s has been successfully run with registry number: %s", jobnetId, rebootJobnetId))

	// Sleep the virtual machine
	err = lib.Sleep_linux(30)
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to sleep your linux os. Sleep count: %d", err.Error(), 30))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Successfully sleep your linux os in bg. Sleep count: %d", 30))
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Reboot Icon not run in 30s."))
	} else {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Reboot Icon run in 30s."))
	}
	...
	if err != nil || reboot_jobnet_run_info == nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error getting reboot jobnet info: %s", err.Error()))
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "%s with registry number %s is completed.", jobnetId, rebootJobnetId))

	// Check jobnet run status and exit code.
	if reboot_jobnet_run_info.Jobnet_status != rebootJobnetStatus {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Unexpected Reboot Jobnet status. Reboot Jobnet_status: %s, Job_status: %s, Exit_cd: %d", reboot_jobnet_run_info.Jobnet_status, reboot_jobnet_run_info.Job_status, reboot_jobnet_run_info.Exit_cd))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Reboot_Jobnet_status: %s, Job_status: %s, Exit_cd: %d", reboot_jobnet_run_info.Jobnet_status, reboot_jobnet_run_info.Job_status, reboot_jobnet_run_info.Exit_cd))
	...
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "%s with registry number %s is completed.", jobnetId, normalJobnetId))

	// Check jobnet run status and exit code.
	if normal_jobnet_run_info.Jobnet_status != normalJobnetStatus {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Unexpected Jobnet status. Jobnet_status: %s, Job_status: %s, Exit_cd: %d", normal_jobnet_run_info.Jobnet_status, normal_jobnet_run_info.Job_status, normal_jobnet_run_info.Exit_cd))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Jobnet_status: %s, Job_status: %s, Exit_cd: %d", normal_jobnet_run_info.Jobnet_status, normal_jobnet_run_info.Job_status, normal_jobnet_run_info.Exit_cd))
...
func RebootAfterCompletingJob_120(jobnetId string, testcase *dao.TestCase, normalJobnetStatus string, normalJobStatus string, rebootJobnetStatus string, rebootJobStatus string, processCheckTimeout int, sshClient *ssh.Client) common.Testcase_status {
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Error: %s, Failed to sleep your linux os. Sleep count: %d", err.Error(), 55))
		return FAILED
	}
	fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Successfully sleep your linux os in bg. Sleep count: %d", 55))
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Reboot icon become warning state with orange color after 1 minute."))
	} else {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Reboot icon become warning state with orange color before 1 minute."))
	}
	...
	if err != nil {
		fmt.Println(lib.Logi(common.LOG_LEVEL_INFO, "Reboot Icon not run in 120s."))
	} else {
		fmt.Println(lib.Logi(common.LOG_LEVEL_ERR, "Reboot Icon run in "))
	}
